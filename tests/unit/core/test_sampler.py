...
def config():
    config = ScoutConfig()
    ScoutConfig.set(
        sample_rate=0.50,  # 50% global sampling
        sample_endpoints={
            "users/test": 0,  # Never sample specific endpoint
            "/users": 1.0,  # Always sample
            "test": 0.20,  # 20% sampling for test endpoints
            "/health": 0,  # Never sample health checks
        },
        sample_jobs={
            "critical-job": 1.0,  # Always sample
            "/batch": 0.30,  # 30% sampling for batch jobs
        },
        ignore_endpoints=["/metrics", "ping"],
        ignore_jobs=["test-job"],
        endpoint_sample_rate=0.70,  # 70% sampling for unspecified endpoints
        job_sample_rate=0.40,  # 40% sampling for unspecified jobs
    )
    yield config
...
def test_should_sample_endpoint_partial(sampler):
    with mock.patch("random.random", return_value=0.10):
        assert sampler.should_sample("Controller/test/endpoint", False) is True
    with mock.patch("random.random", return_value=0.30):
...
def test_should_sample_job_partial(sampler):
    ...
    with mock.patch("random.random", return_value=0.40):
...
def test_should_sample_endpoint_default_rate(sampler):
    ...
    with mock.patch("random.random", return_value=0.80):
...
def test_should_sample_job_default_rate(sampler):
    ...
    with mock.patch("random.random", return_value=0.50):
...
def test_should_handle_legacy_ignore_with_specific_sampling(config):
    ...
    config.set(
        sample_endpoints={
            "foo/bar": 0.50,  # Should override the ignore pattern for specific endpoint
